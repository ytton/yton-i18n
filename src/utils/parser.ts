...
export class Parser {
   ...
   * 解析文件中的硬编码文本
   */
  static async parseHardcodedTexts(document: vscode.TextDocument): Promise<HardcodedText[]> {
   ...
   * 解析Vue文件中的硬编码文本
   */
  private static async parseVueFile(document: vscode.TextDocument, texts: HardcodedText[]): Promise<void> {
    ...
    if (templateMatch) {
      ...
      await this.parseVueTemplate(document, templateContent, templateStartOffset, texts);
    } else {
    ...
    if (scriptMatch) {
      ...
      await this.parseJsContent(document, scriptContent, scriptStartOffset, texts, true);
    } else {
    ...
    while ((styleMatch = styleMatches.exec(text)) !== null) {
      ...
      await this.parseCssContent(document, styleContent, styleStartOffset, styleTexts);
  ...
  private static async parseVueTemplate(
    ...
    while ((match = textRegex.exec(content)) !== null) {
      if (match[1]) {
        ...
        if (cleanedText.length > 1 && !/^\s*$/.test(cleanedText) && 
            !/^\d+$/.test(cleanedText) && !uniqueTexts.has(cleanedText)) {
          uniqueTexts.add(cleanedText);
          const startPos = document.positionAt(startOffset + match.index + 1);
          const endPos = document.positionAt(startOffset + match.index + 1 + rawText.length);
    ...
    for (const attr of hardcodedAttributes) {
      ...
      while ((match = attrRegex.exec(content)) !== null) {
        if (match[1]) {
          ...
          if (attrText.length > 1 && !/^\d+$/.test(attrText) && 
              !['true', 'false'].includes(attrText) && !uniqueTexts.has(attrText)) {
            ...
            const endPos = document.positionAt(startOffset + attrStart + match[1].length);
    ...
    console.log(`Vue模板解析完成，找到 ${texts.length - initialCount} 个硬编码文本`);
  }

  /**
   * 解析JS/TS文件中的硬编码文本
   */
  private static async parseJsFile(document: vscode.TextDocument, texts: HardcodedText[]): Promise<void> {
  ...
  private static async parseJsContent(
    ...
    while ((match = stringRegex.exec(content)) !== null) {
      ...
      if (!text || text.length <= 1 || /^\s*$/.test(text) || 
          /^\d+$/.test(text) || uniqueTexts.has(text)) {
        continue;
      }
      ...
      // 计算位置
      const startPos = document.positionAt(startOffset + match.index + 1); // +1 跳过引号
      const endPos = document.positionAt(startOffset + match.index + match[0].length - 1); // -1 跳过引号
    ...
    console.log(`JS内容解析完成，找到 ${texts.length - initialCount} 个硬编码文本`);
  }

  /**
   * 解析选中区域中的硬编码文本
   */
  static async parseSelectedText(document: vscode.TextDocument, selection: vscode.Selection): Promise<HardcodedText[]> {
    const allTexts = await this.parseHardcodedTexts(document);
    
    // 过滤出选中区域内的硬编码文本
    return allTexts.filter(text => this.rangeIntersects(text.range, selection));
  }

  /**
   * 检查两个Range是否有交集
   */
  private static rangeIntersects(range1: vscode.Range, range2: vscode.Range): boolean {
   ...
   * 解析HTML文件中的硬编码文本
   */
  private static async parseHtmlFile(document: vscode.TextDocument, texts: HardcodedText[]): Promise<void> {
    const content = document.getText();
    const initialCount = texts.length;
    ...
    while ((match = tagContentRegex.exec(content)) !== null) {
      if (match[1]) {
        ...
        if (text && text.length > 1 && !/^\d+$/.test(text) && 
            !/^\s*$/.test(text) && !uniqueTexts.has(text)) {
          uniqueTexts.add(text);
      ...
      while ((match = attrRegex.exec(content)) !== null) {
        if (match[1]) {
          ...
          if (text && text.length > 1 && !/^\d+$/.test(text) && 
            ...
            const endPos = document.positionAt(match.index + attr.length + 2 + match[1].length);
    ...
    console.log(`HTML文件解析完成，找到 ${texts.length - initialCount} 个硬编码文本`);
  }
  ...
  private static async parseCssContent(
    ...
    while ((match = strRegex.exec(content)) !== null) {
      if (match[1]) {
        ...
        if (text && text.length > 1 && !/^\d+$/.test(text) && 
            !/^(https?:\/\/|\/)/.test(text) && !uniqueTexts.has(text)) {
          uniqueTexts.add(text);
          const startPos = document.positionAt(startOffset + match.index + 1);
          const endPos = document.positionAt(startOffset + match.index + 1 + match[1].length);
    ...
    console.log(`CSS内容解析完成，找到 ${texts.length - initialCount} 个硬编码文本`);
  }

  /**
   * 解析CSS文件中的硬编码文本
   */
  private static async parseCssFile(document: vscode.TextDocument, texts: HardcodedText[]): Promise<void> {
    const content = document.getText();
   ...
   * 解析通用文本文件中的硬编码文本
   */
  private static async parseGenericFile(document: vscode.TextDocument, texts: HardcodedText[]): Promise<void> {
    const content = document.getText();
    const initialCount = texts.length;
    ...
    while ((match = doubleQuoteRegex.exec(content)) !== null) {
      if (match[1]) {
        ...
        if (text && text.length > 1 && !/^\d+$/.test(text) && 
            !text.includes('$t(') && !uniqueTexts.has(text)) {
          uniqueTexts.add(text);
          const startPos = document.positionAt(match.index + 1);
          const endPos = document.positionAt(match.index + 1 + match[1].length);
    ...
    console.log(`通用文件解析完成，找到 ${texts.length - initialCount} 个硬编码文本`);
  }
} 