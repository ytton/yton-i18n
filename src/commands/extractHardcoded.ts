...
export async function extractSelection(): Promise<void> {
  ...
  try {
    ...
    // 解析选中区域中的硬编码文本
    const hardcodedTexts = await Parser.parseSelectedText(
      editor.document,
      selection
    );
    ...
    const results = await Transformer.transformHardcodedTexts(hardcodedTexts);
    // 应用转换结果
    await Transformer.applyTransformResults(
      editor.document,
      results,
      hardcodedTexts
    );
...
export async function extractWholeSelectionAsKey(): Promise<void> {
  ...
  // 是否含有引号
  const hasX = /^(['"`]).+\1$/.test(selectedText);
  let keyName = selectedText;
  // 含有引号，则清楚掉引号
  if (hasX) {
    keyName = selectedText.slice(1, -1);
  }
  ...
  // 如果是HTML内容，压缩成一行，防止html内容被格式化成多行
  if (hasHtmlTags) {
    // 简单清理HTML标签获取纯文本内容作为键名的参考
    keyName = selectedText
      .replaceAll(/(\r)?(\n)(\s)*/g, " ") // 移除换行和换行后的空格
      .replaceAll(/\s+/g, " ") // 将多个空白字符替换为单个空格
      .replaceAll(/(<.+?>)(\s+)([,.;])/g, "$1$3")
      .trim();
  }
  ...
  try {
    ...
    // 保存翻译
    const success = TranslationManager.updateKeyTranslation(
      userKey,
      locale,
      userKey //格式化后的key，和原文应该是没多大区别
    );
    if (!success) {
 ...
 * 提取特定硬编码文本（从翻译编辑器中调用）
 */
export async function extractHardcodedText(
  text: string,
  range: any
): Promise<void> {
  ...
  try {
    ...
    if (fileExt === "vue") {
      ...
      if (templateMatch) {
        const templateStart = document.positionAt(
          templateMatch.index + "<template>".length
        );
        const templateEnd = document.positionAt(
          templateMatch.index + templateMatch[0].length - "</template>".length
        );
        ...
        if (
          (textRange.start.line > templateStart.line ||
            (textRange.start.line === templateStart.line &&
              textRange.start.character >= templateStart.character)) &&
          (textRange.start.line < templateEnd.line ||
            (textRange.start.line === templateEnd.line &&
              textRange.start.character <= templateEnd.character))
        ) {
